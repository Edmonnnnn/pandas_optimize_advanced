...
products = pd.read_csv("products.csv").drop_duplicates(subset="product_id").head(500)
purchases = (
    pd.read_csv("purchases.csv", parse_dates=["date"])
    .drop_duplicates(subset=["user_id", "product_id", "date"])
    .query("is_returned == 0 and user_id == user_id")
    .head(5000)
)
discounts = pd.read_csv("discounts.csv", parse_dates=["valid_from", "valid_to"]).drop_duplicates(subset="discount_id").head(50)
